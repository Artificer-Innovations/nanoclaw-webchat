...
| `WEBCHAT_PUBLIC_PATH` | no | — | Public URL path prefix (e.g. `/webchat`) when a reverse proxy stripPrefixes that path; rewrites `/api` and `/assets` in served HTML/JS so the browser stays on the mount |
